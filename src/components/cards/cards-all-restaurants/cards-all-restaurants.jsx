...
export default function CardAllRestaurants({ filteredData }) {
  ...
  return (
    <Grid container spacing={2} style={{ padding: 10, paddingBottom: 15 }}>
      {isFilteredDataEmpty ? (
        // Se não houver dados filtrados, renderiza todos os cards de dataOfAllRestaurants
        ...
        // Se houver dados filtrados, renderiza os cards filtrados
        filteredData.map(({ id, title: childName, status, image, routePage }) => (
          <Grid item xs={12} sm={6} md={4} key={id}>
            {status === "Aberto" && (
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Typography>{childName}</Typography>
